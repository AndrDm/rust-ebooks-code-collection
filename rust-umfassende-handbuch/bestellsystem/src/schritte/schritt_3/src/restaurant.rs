...
pub use speisekarte::Speisekarte;
use crate::restaurant;

use self::{
    gast::Gast,
    mitarbeiter::{Mitarbeiter, MitarbeiterId},
    tisch::{Tisch, TischId},
};
...
/// Das Restaurant verbindet [`Mitarbeiter`] und [Tische](restaurant::tisch::Tisch).
/// Es weist außerdem [Öffnungszeiten][Ö] aus.
///
/// [Ö]: Restaurant::oeffnungszeiten
...
impl Restaurant {
    ...
    pub fn new(
        ...
        if tisch_zuordnung.is_empty() {
            panic!("Kann ein leeres Restaurant nicht öffnen")
        }
...
// Einen Test mit "path" verschieben
// #[cfg(test)]
...
mod tests {
    ...
    fn ein_mitarbeiter_wird_zugewiesen() {
        ...
        if let GastZustand::HatTischErhalten(tisch) = zustand {
            ...
            panic!("Der Gast hätte einen Tisch erhalten sollen");
        }
    }
    ...
    mod oeffnungszeiten_tests {
        // ...
        #[test]
        fn nicht_leer() {}
    }

    mod statische_pruefungen {
        use super::super::*;

        macro_rules! ist {
            ($zu_testen:ty, $erwartet:ident) => {
                // Wichtig, der Block muss die Deklaration
                // auf das Makro beschränken!
                {
                    fn _ist<T: $erwartet>() {}
        ...
        fn restaurant_ist_partial_eq() {
            // Wird nur kompilieren, wenn wahr
            // ist!(Restaurant, PartialEq);
            // Clone ebenfalls prüfen
            // ist!(Restaurant, Clone);
            // ...
        }
    }
}
